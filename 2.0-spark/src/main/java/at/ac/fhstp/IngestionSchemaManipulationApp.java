...
public class IngestionSchemaManipulationApp {
        ...
        public Dataset ingestJSON(SparkSession spark, String source) {
                Dataset<Row> df = spark.read().format("json").option("header", "true").load(source);

                // Tranform the data
                df = df.withColumn("county", lit("Durham")).withColumn("datasetId", df.col("fields.id"))
                ...
                df = df.drop("fields").drop("record_timestamp").drop("recordid").drop("geometry").drop("closing_date");
                df = df.withColumn("id",
        ...
        public Dataset ingestCSV(SparkSession spark, String source) {
                ...
                Dataset<Row> df = spark.read().format("csv").option("header", "true").load(source);

                df = df.withColumn("county", lit("Wake")).withColumnRenamed("HSISID", "datasetId")
                                .withColumnRenamed("NAME", "name").withColumnRenamed("ADDRESS1", "address1")
                                .withColumnRenamed("ADDRESS2", "address2").withColumnRenamed("CITY", "city")
                                .withColumnRenamed("STATE", "state").withColumnRenamed("POSTALCODE", "zip")
                                .withColumnRenamed("PHONENUMBER", "tel")
                                .withColumnRenamed("RESTAURANTOPENDATE", "dateStart")
                                .withColumnRenamed("FACILITYTYPE", "type").withColumnRenamed("X", "geoX")
                                .withColumnRenamed("Y", "geoY").drop("OBJECTID").drop("PERMITID").drop("GEOCODESTATUS");
